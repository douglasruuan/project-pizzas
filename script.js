let quantidade = 1; //Quantidade inicial modelo
let carrinho = []; //Array Carrinho
let qualPizza = 0; //Qual a pizza selecionada no modelo
...
pizzasJson.map((valorAtual, indice) => {
    ...
    let pizzaItem = c('.modelos .pizza-item').cloneNode(true); //cloner modelo itens pizza

    pizzaItem.setAttribute('data-key', indice); //Adiciona atributo especifico a elemento e fornece valor especifico

    //Adiciona dados pizza
